...
FROM openjdk:11
EXPOSE 8080
ADD target/springboot-inbox-app-0.0.1-SNAPSHOT.jar springboot-inbox-app.jar
ENTRYPOINT ["sh","-c","java -jar springboot-inbox-app.jar"]
...
EXPOSE 8080
ENTRYPOINT ["java","-cp","app:app/lib/*","com.springboot.inbox.InboxMainApp"]
